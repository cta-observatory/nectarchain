...
def make_summary_card(file, display_registry, parentkeys, childkeys, run_index=-1):
    ...
    layout_id = np.where(
        file[parentkeys["layout_parentkey"]][childkeys["tel_id"]] == tel_id
    )[0][0]
    trigger_id = np.where(
        file[parentkeys["trigger_parentkey"]][childkeys["event_id"]] == event_id
    )[0][0]
    name = file[parentkeys["layout_parentkey"]][childkeys["name"]][layout_id].decode(
        "utf-8"
    )
    camera_type = file[parentkeys["layout_parentkey"]][childkeys["camera_type"]][
        layout_id
    ].decode("utf-8")
    time = file[parentkeys["trigger_parentkey"]][childkeys["time"]][trigger_id]
    timestamp_qns = file[parentkeys["trigger_parentkey"]][childkeys["timestamp_qns"]][
        trigger_id
    ]
    ...
    event_type = file[parentkeys["parameter_parentkey"]][childkeys["event_type"]][
        run_index
    ]
    event_quality = file[parentkeys["parameter_parentkey"]][childkeys["event_quality"]][
        run_index
    ]
    event_goodness = file[parentkeys["parameter_parentkey"]][
    ...
    try:
        if hasattr(disp, "children") and len(disp.children) > 1:
            if hasattr(disp.children[1], "text"):
                ...
                event_id = file[parentkeys["parameter_parentkey"]][
                    childkeys["event_id"]
                ][run_index]
                layout_id = np.where(
                    file[parentkeys["layout_parentkey"]][childkeys["tel_id"]] == tel_id
                ...
                trigger_id = np.where(
                    file[parentkeys["trigger_parentkey"]][childkeys["event_id"]]
                    == event_id
                )[0][0]
                name = file[parentkeys["layout_parentkey"]][childkeys["name"]][
                    layout_id
                ].decode("utf-8")
                camera_type = file[parentkeys["layout_parentkey"]][
                    childkeys["camera_type"]
                ][layout_id].decode("utf-8")
                time = file[parentkeys["trigger_parentkey"]][childkeys["time"]][
                    trigger_id
                ]
                timestamp_qns = file[parentkeys["trigger_parentkey"]][
                    childkeys["timestamp_qns"]
                ][trigger_id]
                alt_tel = file[parentkeys["parameter_parentkey"]][childkeys["alt_tel"]][
                    run_index
                ]
                az_tel = file[parentkeys["parameter_parentkey"]][childkeys["az_tel"]][
                    run_index
                ]
                obs_id = file[parentkeys["parameter_parentkey"]][childkeys["obs_id"]][
                    run_index
                ]
                event_type = file[parentkeys["parameter_parentkey"]][
                    childkeys["event_type"]
                ][run_index]
                event_quality = file[parentkeys["parameter_parentkey"]][
                    childkeys["event_quality"]
                ][run_index]
                event_goodness = file[parentkeys["parameter_parentkey"]][
                    childkeys["is_good_event"]
                ][run_index]
