#HOW TO:
#In order to compute flat-field coefficients using the gain computed with photostatistincs method, one has to:
...
#2) Run the phototatistic method and the fit of 2D Gaussian model to get ff-coeficients
