...
import numpy as np
from nectarchain.data.container import WaveformsContainers, WaveformsContainer
from ctapipe.instrument import SubarrayDescription
from ctapipe.containers import EventType
import os
from nectarchain.data.container import ChargesContainers, ChargesContainer
...
class TestBenchNectarchainConverterTool():
    ...
    def extract_file_details(self):
        file_dir = os.path.join(os.environ['NECTARCAMDATA'], f'testbenchrun/RUN_{self.run_number}/')
        ...
        for file in files:
            ...
            drawer,fileno,evtstotal =split_file[2],int(split_file[3].split(".")[0]),int(split_file[1])
    ...
    def create_irap_waveformContainer(self,wfs):
        ...
        waveform_container = WaveformsContainer(
            ...
            camera="IRAP-TB",
            broken_pixels_hg=np.full(
                shape=(nevents, npix), fill_value=0, dtype=bool
            ),
            broken_pixels_lg=np.full(
                shape=(nevents, npix), fill_value=0, dtype=bool
            ),
            ucts_timestamp=rng.integers(low=0, high=nevents, size=(nevents), dtype=np.uint64),
    ...
    def run_tool(self):
        ...
        if self.events_per_slice > evtspfile:
            raise Exception("events per slice must be equal to or less than the events per test bench file")
        ...
        for fileno in range(noffiles):
            ...
            while combined.shape[1] >= self.events_per_slice and written_total < self.max_events:
