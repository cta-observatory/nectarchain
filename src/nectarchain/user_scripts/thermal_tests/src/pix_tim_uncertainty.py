...
def main():
    ...
    filename = (
        output_dir + "pix_time_uncer_data.txt"
    ...
    with open(filename, "w") as f:
        ...
        writer.writerow(
            ["Run", "photons_spline", "Mean_RMS", "rms_no_fit_weighted_err"]
        )
        ...
        for run in range(len(runlist)):
            ...
            rms_no_fit_weighted_err.append(
                np.sqrt(1 / np.nansum(weights_no_fit_pix[run]))
            )
            runname = runlist[run]
            writer.writerow(
                [
                    runname,
                    float(photons_spline[run]),
                    np.sqrt(np.array(rms_no_fit_weighted) ** 2),
                    np.sqrt(1 / np.nansum(weights_no_fit_pix[run])),
                ]
            )
