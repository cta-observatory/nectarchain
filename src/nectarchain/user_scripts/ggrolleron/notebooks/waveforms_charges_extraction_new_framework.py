...
# %%
tool = WaveformsNectarCAMCalibrationTool(progress_bar = True,run_number = run_number,max_events = 500,log_level = 10)
tool
...
with HDF5TableReader(f"/tmp/EventsLoopNectarCAMCalibrationTool_{run_number}.h5") as reader : 
    for key,trigger in trigger_type.items() : 
        try : 
            tableReader = reader.read(table_name = f"/data/WaveformsContainer_0/{trigger.name}", containers = WaveformsContainer)
...
with HDF5TableReader(f"/tmp/EventsLoopNectarCAMCalibrationTool_{run_number}.h5") as reader : 
    for key,trigger in trigger_type.items() : 
        try : 
            tableReader = reader.read(table_name = f"/data/ChargesContainer_0/{trigger.name}", containers = ChargesContainer)
            container.containers[trigger] = next(tableReader)
        except Exception as err:
            print(err)
