...
do
    ...
    then
        export cmd="python load_wfs_compute_charge.py -r 3938 --events_per_slice 2000 --method $METHOD  --overwrite -v INFO --reload_wfs"
        echo $cmd
        eval $cmd
    else
        for WIDTH in 8 9 10 11 12 16
        do
            export cmd="python load_wfs_compute_charge.py -r 3938 --events_per_slice 2000 --method $METHOD --extractor_kwargs '{\"window_width\":$WIDTH,\"window_shift\":4}' --overwrite -v INFO"
            echo $cmd
            eval $cmd
        done
    fi
