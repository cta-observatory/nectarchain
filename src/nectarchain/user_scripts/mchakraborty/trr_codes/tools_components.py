...
def _init_output_path(self):
    ...
    if self.max_events is None:
        filename = f"{self.output_dir}/{self.name}_run{self.run_number}.h5"
    else:
        filename = f"{self.output_dir}/{self.name}_run{self.run_number}_maxevents{self.max_events}.h5"
...
class ChargeComp(NectarCAMComponent):
    ...
    def __call__(self, event: NectarCAMDataContainer, *args, **kwargs):
        ...
        for i, (pedestal, charge) in enumerate(
            ...
            """
            bad_modules1 = [il for il in range(0,9)]
            i1 = 8

            bad_modules2 = []
            for il in range(11,19):
                bad_modules2.append(i1)

                i1 = i1+il
            bad_modules2 = np.array(bad_modules2).flatten()
            bad_modules3 = [141,159,176,192,207,221,234,246]

            bad_modules4 = [il for il in range(257,266)]
            bad_modules5 = [18,30,43,57,72,88,105,123]
            bad_modules6 = [158,175,191,206,220,233,245,256]
            """
            ...
            if (
                ...
                for pix in range(len(self.pixels_id)):
                    ...
                    # if(module in bad_modules1 or module in bad_modules2 or module in bad_modules3 or module in bad_modules4 or module in bad_modules5 or module in bad_modules6 or pix==827):
                    # integral[pix] = np.nan
                    # continue
                    ...
                    if (
                        ...
                        # print(self.__tom_mean[pix],self.__tom_std[pix],index_peak[pix], pix, event.index.event_id)
                        integral[pix] = np.nan

                    """
                    if(mean_max<260 and i==0):
                        ...
                        #print("range ",self.__tom_mean[pix]-5,self.__tom_mean[pix]+5,integral[pix],pix,ped)
                    """
                    # if(integral[pix]<-70):
                    # print(i,integral[pix],event.count,event.index.event_id,ped," pix ",pix,signal_start[pix],signal_stop[pix],event.trigger.event_type," ToM ", index_peak[pix])
                    # integral[pix]=0

                    # if(event.index.event_id==1832):
                    # print(" 1832 ",mean_max,self.__mean_pe, self.__tom_mean[pix], index_peak[pix],integral[pix], pix, event.index.event_id)
                chg = integral
                # print(np.mean(mean_max) , np.max(mean_max),mean_max)
                # print("integral ",mean_max,  np.nanmean(integral), len(integral), event.index.event_id, np.isnan(integral).sum())
...
class LinearityTestTool(EventsLoopNectarCAMCalibrationTool):
    ...
    def finish(self, *args, **kwargs):
        ...
        return mean_charge, std_charge, std_err, npixels, tom_mean
...
class ChargeResolutionTestTool(EventsLoopNectarCAMCalibrationTool):
    ...
    """ 
    def set_thermal_params(temperature=None,ff_true=True):
        self.temperature = temperature
        self.ff_true     = ff_true
    """
    ...
    def finish(self, *args, **kwargs):
        ...
        # print("min ", np.min(np.concatenate(charge_pe_lg)), np.min(np.concatenate(charge_pe_hg)))

        """
        fig,axs = plt.subplots(1,2)
        axs[0].set_yscale('log')
        axs[0].set_xlabel("Charge LG (pe)")
        axs[0].hist(charge_pe_lg.flatten(),bins=1000,range=[-10.,100])
        axs[1].set_yscale('log')
        axs[1].set_xlabel("Charge HG (pe)")
        axs[1].set_ylim(0.01,pow(10,5))
        axs[1].hist(charge_pe_hg.flatten(),bins=1000,range=[-10.,700])
