...
def get_args():
    ...
    parser.add_argument('-r','--runlist', type=int, nargs='+', help='List of runs (numbers separated by space)', required=False, default = [i for i in range (3446,3457)])
    parser.add_argument('-e','--evts', type = int, help='Number of events to process from each run. Default is 100', required=False, default=100)
...
def main():
    ...
    for run in runlist:
        ...
        tool = TimingResolutionTestTool(
            progress_bar=True, run_number=run, max_events=nevents, events_per_slice = 999, log_level=20, window_width=16, overwrite=True
        )
        tool.initialize()
        tool.setup()
        tool.start()
