...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="List of runs (numbers separated by space). You can put just one run, default 3292",
        required=False,
        default=[3292],
    )
    parser.add_argument(
        ...
        "--evts",
        type=int,
        help="Number of events to process from each run. Default is 100. 1000 or more gives best results but takes some time",
        required=False,
        default=100,
    )
    parser.add_argument(
        ...
        required=False,
        default="../transit_time/hv_pmt_tom_correction_laser_measurement_per_pixel_fit_sqrt_hv_newmethod.csv",
    )
    parser.add_argument(
        "-o",
        "--output",
