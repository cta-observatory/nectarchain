...
import numpy as np
from test_tools_components import ToMPairsTool
import os
import sys
import pickle
...
def get_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Time resolution (timing uncertainty between couples of pixels) test B-TEL-1030.\n'
    ...
    parser.add_argument('-r','--runlist', type=int, nargs='+', help='List of runs (numbers separated by space). You can put just one run, default 3292', required=False, default = [3292])
    parser.add_argument('-e','--evts', type = int, help='Number of events to process from each run. Default is 100. 1000 or more gives best results but takes some time', required=False, default=100)
    parser.add_argument('-t','--pmt_transit_time', type=str, help='.csv file with pmt transit time corrections', required=False, default="/Users/dm277349/nectarchain_data/transit_time/hv_pmt_tom_correction_laser_measurement_per_pixel_fit_sqrt_hv_newmethod.csv")
...
def main():
    ...
    tt_path = "/Users/dm277349/nectarchain_data/transit_time/hv_pmt_tom_correction_laser_measurement_per_pixel_fit_sqrt_hv_newmethod.csv"
    ...
    for run in runlist:
        ...
        tool = ToMPairsTool(
            progress_bar=True, run_number=run, events_per_slice = 501 ,max_events=nevents, log_level=20, peak_height=10, window_width=16, overwrite=True
        )
        tool.initialize()
        tool.setup()
        tool.start()
    ...
    plt.hist(std_corrected,range=(0,5),density=True, 
                histtype='step', lw=3, bins=200)

    plt.axvline(2, color='C4', alpha=0.8)
    ax.text(2.1, 0.5, 'CTA requirement', color='C4', fontsize=20,
            rotation=-90)


    ax.annotate("", xy=(1.6, 0.25), xytext=(2, 0.25), color='C4', alpha=0.5,
                transform=ax.transAxes,
                arrowprops=dict(color='C4', alpha=0.7, lw=3, arrowstyle='->'))

    ax.annotate("", xy=(1.6, 0.75), xytext=(2, 0.75), color='C4', alpha=0.5,
                transform=ax.transAxes,
                arrowprops=dict(color='C4', alpha=0.7, lw=3, arrowstyle='->'))

    plt.xlabel('RMS of $\Delta t_{\mathrm{TOM}}$ for pairs of pixels [ns]')
