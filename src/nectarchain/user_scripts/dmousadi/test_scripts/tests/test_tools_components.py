...
class ChargeComp(NectarCAMComponent):
    ...
    def __call__(self, event: NectarCAMDataContainer, *args, **kwargs):
        ...
        #####THE JOB IS HERE######
        for i, (pedestal,charge) in enumerate(zip([self.__pedestal_hg, self.__pedestal_lg],[self.__charge_hg,self.__charge_lg])):
            ...
            if event.trigger.event_type==EventType.FLATFIELD:
                ...
                for pix in range(len(self.pixels_id)):
                    ...
                    peaks_sat = find_peaks(wf[pix,20:45],height=1000,plateau_size=self.window_width)
                    if len(peaks_sat[0])==1:
                        #saturated
                        
                        #print("saturated")
                        
                        signal_start[pix]=argmedian(wf[pix,20:45])+20-self.window_shift
                        signal_stop[pix]=signal_start[pix]+self.window_width
                        integral[pix] = np.sum(wf[pix,signal_start[pix]:signal_stop[pix]])-ped*(signal_stop[pix]-signal_start[pix])
    ...
    def finish(self):
        ...
        output = ChargeContainer(
            ...
            event_type=ChargeContainer.fields["event_type"].dtype.type(self.__event_type),
...
class ToMContainer(NectarCAMContainer):
    ...
    charge_hg = Field(
        type=np.ndarray, dtype=np.float64, ndim=2, description="The mean high gain charge per event"
    )
    ...
    # )
    tom_no_fit = Field(
        type=np.ndarray, dtype=np.float64, ndim=2, description="Time of maximum from data (no fitting)"
    )
    good_evts = Field(type=np.ndarray, dtype=np.uint32, ndim=1, description="good (non cosmic ray) event ids")
...
class ToMComp(NectarCAMComponent):
    ...
    def __call__(self, event: NectarCAMDataContainer, *args, **kwargs):
        ...
        for i, (pedestal,charge,tom_no_fit) in enumerate(zip([self.__pedestal_hg],[self.__charge_hg],[self.__tom_no_fit])):
            ...
            ped = np.array([np.mean(wf[pix, 0:signal_start[pix]]) for pix in range(len(self.pixels_id))])
            ...
            for pix in range(len(self.pixels_id)):
                y = wf[pix] - ped[pix]#np.maximum(wf[pix] - ped[pix],np.zeros(len(wf[pix])))
                ...
                yi = ius(xi)
                peaks, _ = find_peaks(yi, height=self.peak_height, )
                #print(peaks)
                ...
                if len(peaks)>0:
                    ...
                    #saturated events
                    if ((len(maxima_peak_index) > 1) and (min(xi[peaks[maxima_peak_index]]) > signal_start[pix]) and (
                            max(xi[peaks[maxima_peak_index]]) < signal_stop[pix])):
                        # saturated event
                        ...
                        x_max = closest_value(x, xi[peaks[max_peak_index]])  # find the maximum in the not splined array
                        charge_sum = y[(list(x).index(x_max) - (self.window_width - 10)):(list(x).index(x_max) + (self.window_width - 6))].sum()
                    ...
                    elif ((len(maxima_peak_index) == 1) and (xi[peaks[max_peak_index]] > signal_start[pix]) and (
                            xi[peaks[max_peak_index]]< signal_stop[pix])):
                        ...
                        charge_sum = y[signal_start[pix]:signal_start[pix] + self.window_width].sum()
    ...
    def finish(self):
        ...
        output = ToMContainer(
            ...
            charge_hg=ToMContainer.fields["charge_hg"].dtype.type(
                self.__charge_hg
            ),
            ...
            tom_no_fit=ToMContainer.fields["tom_no_fit"].dtype.type(
                self.__tom_no_fit
            ),
            
            good_evts=ToMContainer.fields["good_evts"].dtype.type(
                self.__good_evts
            ),
...
class TimingResolutionTestTool(EventsLoopNectarCAMCalibrationTool):
    ...
    def finish(self, bootstrap=False, *args, **kwargs):
        ...
        tom_no_fit = np.array(tom_no_fit_all[good_evts]).reshape(len(good_evts),npixels)
        #print(tom_no_fit)
        #print(tom_no_fit)
        ...
        for pix in range(npixels):
            ...
            for tom, rms, err in (zip([tom_no_fit[:,pix]],[rms_no_fit],[rms_no_fit_err])):
                tom_pos = tom[tom>20]
                boot_rms = []
                ...
                if bootstrap:
                    ...
                    if len(sample)!=0:
                        for _ in range(1000):
                            bootsample = np.random.choice(sample,size=int(3/4*(len(sample))), replace=True)
                #             print(len(bootsample), bootsample.mean(), bootsample.std())
                    ...
                    try:
                        ...
                        weighted_variance = np.average((bin_centers - weighted_mean)**2, weights=hist_values)
...
class ToMPairsTool(EventsLoopNectarCAMCalibrationTool):
    ...
    def finish(self, *args, **kwargs):
        ...
        tom_corrected = -np.ones(tom_no_fit.shape,dtype=np.float128)  #-1 for the ones that have tom beyond 0-60

        iter = enumerate(pixels_id)

        for i,pix in iter:
            #print(pix, pmt_tt[pix])
            normal_values = [a and b for a,b in zip(tom_no_fit[:,i]>0,tom_no_fit[:,i]<60)]
            
            tom_corrected[normal_values,i] = tom_no_fit[:,i][normal_values]-pmt_tt[pix]
            
            #print(tom_corrected)

        pixel_ind = [i for i in range(len(pixels_id))] #dealing with indices of pixels in array
        ...
        for i,pair in enumerate(pixel_pairs):
            ...
            for event in range(tom_no_fit_all.shape[0]):
                cond_no_correction = (tom_no_fit[event,pix1_ind]>0 and tom_no_fit[event,pix1_ind]<60
                                      and tom_no_fit[event,pix2_ind]>0 and tom_no_fit[event,pix2_ind]<60)
                cond_correction = (tom_corrected[event,pix1_ind]>0 and tom_corrected[event,pix1_ind]<60
                                      and tom_corrected[event,pix2_ind]>0 and tom_corrected[event,pix2_ind]<60)

                if cond_no_correction: #otherwise will be nan
                    
                    dt_no_correction[i,event] = tom_no_fit[event,pix1_ind] - tom_no_fit[event,pix2_ind]
                ...
                if cond_correction:
                    ...
                    dt_corrected[i,event] =  tom_corrected[event,pix1_ind] - tom_corrected[event,pix2_ind]
        ...
        return  tom_no_fit,tom_corrected,pixels_id, dt_no_correction, dt_corrected, pixel_pairs
...
class PedestalContainer(NectarCAMContainer):
    ...
    pedestal_hg = Field(
        type=np.ndarray, dtype=np.float64, ndim=2, description="High gain pedestal per event"
    )

    pedestal_lg = Field(
        type=np.ndarray, dtype=np.float64, ndim=2, description="Low gain pedestal per event"
    )

    rms_ped_hg = Field(
        type=np.ndarray, dtype=np.float64, ndim=2, description="High gain pedestal rms per event"
    )

    rms_ped_lg = Field(
        type=np.ndarray, dtype=np.float64, ndim=2, description="Low gain pedestal rms per event"
    )
...
class PedestalComp(NectarCAMComponent):
    ...
    def __call__(self, event: NectarCAMDataContainer, *args, **kwargs):
        ...
        for i, (pedestal,rms_pedestal) in enumerate(zip([self.__pedestal_hg,self.__pedestal_lg],[self.__rms_ped_hg,self.__rms_ped_lg])):
            wf = np.array(wfs[i]) #waveform per gain
    ...
    def finish(self):
        ...
        output = PedestalContainer(
            ...
            event_type=PedestalContainer.fields["event_type"].dtype.type(self.__event_type),
...
class UCTSContainer(NectarCAMContainer):
    ...
    event_id = Field(type=np.ndarray, dtype=np.uint32, ndim=1, description="event ids")
    ucts_busy_counter = Field(type=np.ndarray, dtype=np.uint32, ndim=1, description="busy counter")
    ucts_event_counter = Field(type=np.ndarray, dtype=np.uint32, ndim=1, description="event counter")
...
class DeadtimeTestTool(EventsLoopNectarCAMCalibrationTool):
    ...
    def finish(self, *args, **kwargs):
        ...
        #print(ucts_timestamps)
        delta_t = [ucts_timestamps[i]-ucts_timestamps[i-1] for i in range(1,len(ucts_timestamps))]
        ...
        return  ucts_timestamps,delta_t,event_counter,busy_counter,collected_trigger_rate, time_tot, deadtime_pc
