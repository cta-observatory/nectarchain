...
filters=np.array(
    [
        [0.00, 0.15, 0.30, 0.45, 0.60, 0.75, 0.90, 1.00, 1.15, 1.30, 1.30, 1.45, 1.50, 1.60, 1.65, 1.80, 1.90, 2.00, 2.10, 2.15, 2.30, 2.30, 2.50, 2.60, 2.80, 3.00, 3.30, 3.50],
        [0.000000, 0.213853, 0.370538, 0.584391, 0.739449, 0.953302, 1.109987, 1.344491, 1.558344, 1.715029, 1.792931, 2.006784, 2.089531, 2.163469, 2.303384, 2.460068, 2.532380, 2.695312, 2.828980, 2.909165, 3.065849, 3.137422, 3.434022, 3.434761, 3.882462, 4.039803, 4.488243, 4.784842],
        [1.000000, 0.611149, 0.426052, 0.260381, 0.182201, 0.111352, 0.077627, 0.045239, 0.027647, 0.019274, 0.016109, 0.009845, 0.008137, 0.006863, 0.004973, 0.003467, 0.002935, 0.002017, 0.001483, 0.001233, 0.000859, 0.000729, 0.000368, 0.000367, 0.000131, 0.000091, 0.000032, 0.000016]       
    ]
).T
...
trasmission_390ns = np.array([0.002016919,
0.045238588,
0.0276475,
0.019273981,
0.008242516,
...
optical_density_390ns = np.array([2.69531155,
1.34449096,
1.55834414,
1.71502857,
2.08394019,
...
intensity_percent = np.array([13. ,          15. ,         16. ,         16.5, 
                20.6,                   22. , 
                25. ,         35. ,          33])
intensity_to_charge =np.array([  1.84110824,   2.09712394,   2.24217532,   2.37545181,
                10.32825426,   28.80655155,
            102.79668643, 191.47895686, 188])


source_ids_deadtime = [0 for i in range(3332,3342)] + [1 for i in range(3342,3350)] + [2 for i in range(3552,3562)]

deadtime_labels = {0:{'source':"random generator",'color':'blue'},
          1:{'source':"nsb source", 'color':'green'},
          2:{'source':"laser",'color':'red'}
}


def pe_from_intensity_percentage (percent,percent_from_calibration=intensity_percent,known_charge=intensity_to_charge):
    """
...
def err_ratio(nominator, denominator, err_norm, err_denom, cov_nom_den = 0):
    ...
    """
    delta_err2 = (err_norm/nominator)**2 + (err_denom/denominator)**2 -2*cov_nom_den/(nominator*denominator)
...
def closest_value(input_list, input_value):
    """
    Returns the value in `input_list` that is closest to `input_value`.
    
    Args:
        input_list (list): The list of values to search.
        input_value (float): The value to find the closest match for.
    
    Returns:
        float: The value in `input_list` that is closest to `input_value`.
    """
...
def gaus(x,a,mu,sigma):
    """
    Computes the Gaussian function with the given parameters.
    ...
    Returns:
        float: The value of the Gaussian function at the given input `x`.
    """
...
class ExponentialFitter():
    ...
    def compute_minus2loglike(self,x):
        ...
        chi2_mask = expected>0.
        minus2loglike   = -2.*np.sum( poisson.logpmf(self.datas[chi2_mask],mu=expected[chi2_mask]) )
        minus2loglike0  = -2.*np.sum( poisson.logpmf(self.datas[chi2_mask],mu=self.datas[chi2_mask]) )
...
def deadtime_and_expo_fit(time_tot, deadtime_us,run, output_plot = None):
    ...
    deadtime_err = (pucts[1][first_nonemptybin] - pucts[1][first_nonemptybin - 1]) /\
    ...
    entries, bin_edges = np.histogram(data, bins=bins, range=[0, total_delta_t_for_busy_time])
    ...
    if output_plot:
        ...
        plt.hist(data, bins= np.arange(1000) - 0.5,
                alpha=0.8,
                histtype='step', density=0, lw=2, label='Run {}'.format(run))
        ...
        plt.plot(
            ...
            marker='', linestyle='-', lw=3, color='C3', alpha=0.85,
        ...
        ax.text(600, entries[1] / 1 , '$y = A \cdot \exp({-R \cdot x})$\n'
                #          + r'$A=%2.2f \pm %2.2f$'%(as_si((result.params['A'].value/1000)*1e3,2), as_si((result.params['A'].stderr/1000)*1e3,2))
                + r'$A=%2.2f \pm %2.2f$' % (result.params['A'].value, result.params['A'].stderr)
                + '\n' + r'$R=(%2.2f \pm %2.2f)$ kHz' % (R, R_stderr)
                + '\n' + r'$\chi^2_\nu = %2.2f$' % ((result.redchi))
                + '\n' + r'$\delta_{\mathrm{deadtime}} = %2.3f \, \mu$s' % (deadtime),
                backgroundcolor='white', bbox=dict(facecolor='white', edgecolor='C3', lw=2,
                                                boxstyle='round,pad=0.3'),
                ha='left', va='top', fontsize=17, color='k', alpha=0.9)


        plt.xlabel(r'$\Delta$T [$\mu$s]')
        plt.ylabel('Entries')
        plt.title('Run {}'.format(run), y=1.02)
        plt.savefig(os.path.join(output_plot,"deadtime_and_expo_fit_{}.png".format(run)))


    return deadtime, deadtime_bin, deadtime_err, deadtime_bin_length, \
           total_delta_t_for_busy_time, parameter_A_new, parameter_R_new, \
           parameter_A_err_new, parameter_R_err_new, \
