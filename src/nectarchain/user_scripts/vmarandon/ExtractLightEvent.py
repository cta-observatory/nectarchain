try: 
    ...
    from Utils import GetCamera, GetDefaultDataPath, SignalIntegration, IntegrationMethod, save_simple_data, LightEvent, CustomFormatter
    ...
    from Stats import CameraStats, CameraSampleStats, Stats
...
def half_max_x(x, y, maxfrac = 0.5):
    ...
    if len(zero_crossings_i)>0:
        try:
            delta = [lin_interp(x, y, zero_crossings_i[0], half),
...
def half_max_x_fast(x, y, maxfrac = 0.5):
    ...
    if len(zero_crossings_i)>=2:
        try:
            delta = [lin_interp_fast(x, y, zero_crossings_i[0], half),
...
def ExtractLightEvent(arglist):
    p = argparse.ArgumentParser(description='Extract events according to criteria',
                                epilog='examples:\n'
    ...
    p.add_argument("--ff-no-internal",dest="ffNoInternal",action='store_true',help="Tell the code that we are dealing with a FF run, having real illumination but no internal trigger")
    p.add_argument("--internal-ff",dest="internalFF",action='store_true',help='Tell the code that the FF event are only in internal mode so it has to guess if the event is a FF')
    p.add_argument("--ff-npixel-threshold",dest="ffNPixThreshold",type=int,default=1800,help="Number of pixel to declare a ff valid")
    p.add_argument("--r0",dest="saveR0",action='store_true',help='Save the r0 for all events')
    p.add_argument("--telid",dest="telId",type=int,default=0,help="Telescope id for which we show the data distributions")
    p.add_argument("--ped-length",dest='pedLength',type=int,default=16,help="Number of slice to use for pedestal estimation at the beginning of each event")
    p.add_argument("--noise-thresh",dest='noiseThresh',type=float,default=1e6,help="Threshold to filter out noise in waveform (only useful if no HV)")
    ...
    run=args.run
    return ExtractLightEventForRun( run=run, path=path, nEvents=args.nEvents, ffNPixThreshold=args.ffNPixThreshold, ffNoInternal=args.ffNoInternal, internalFF=args.internalFF, saveR0=args.saveR0, telId=args.telId,noiseThresh=args.noiseThresh,pedLength=args.pedLength)

def  ExtractLightEventForRun(run, path, nEvents=-1, ffNPixThreshold=1800, ffNoInternal=False,internalFF=False,saveR0=False,telId=0,noiseThresh=1e6,pedLength=16):
    ...
    with PickleDataWriter(f'run_{run}_lightevents.pickle.lz4') as outFile:
        ...
        for i, evt in enumerate(tqdm(data,total=nMax)):
            ...
            waveform = evt.r1.tel[telId].waveform if R1Data else evt.r0.tel[telId].waveform

            trigger = copy.deepcopy( evt.trigger )

            nectarcam = copy.deepcopy( evt.nectarcam )
            nectarcam.tel[telId].evt.trigger_pattern = np.any(evt.nectarcam.tel[telId].evt.trigger_pattern,axis=0)
            ffcandidate = internalFF and IsFFEventCandidate(nectarcam.tel[telId].evt.trigger_pattern,ffNPixThreshold)
            ...
            npix = np.count_nonzero(np.any(evt.nectarcam.tel[telId].evt.trigger_pattern,axis=0))
            ...
            if ped_mask is None:
                ped_mask = np.tile( np.arange(waveform.shape[-1]), waveform.shape[:-1] + (1,) ) < pedLength
            ...
            le.saturated.tel[telId] = np.any( waveform > 4000, axis=-1 ) # In principle 4095 but I put a safety margin

            integration_method = None
            if evt.trigger.event_type == EventType.SKY_PEDESTAL:
                integration_method = None
            elif (evt.trigger.event_type == EventType.FLATFIELD or ffcandidate) and (npix>ffNPixThreshold or ffNoInternal):
            ...
            if integration_method is not None:
                ...
                evt_charge, evt_timing = SignalIntegration(excess_wvf,hdw_fail_wvf,method=integration_method,left_bound=5,right_bound=9,camera=camera,peakpositions=None)
            
                excess_wvf[hdw_fail_wvf] = 0.
            ...
            if evt.trigger.event_type == EventType.SKY_PEDESTAL:
                ...
                mean_wvf[ bad_hdw | noisy ] = 0.
                le.pedestal.tel[telId] = mean_wvf.copy()
            elif evt.trigger.event_type == EventType.FLATFIELD and npix < 30 and not ffNoInternal: # 30 is arbitrary, I don't know
