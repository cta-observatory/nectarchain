...
class PedestalInfo(TimedInfo):
    ...
    def ShowTraceInfo(self, pixid, showMinMax=True, entryId=None):
        ...
        try:
            fig, axs = plt.subplots(
                nrows=1, ncols=ped_shape[0], figsize=(ped_shape[0] * 6, 6)
            )
            ...
            for chan in range(2):
                ...
                axs[chan].fill_between(
                    x, pos - width, pos + width, color=color, alpha=0.5, label="Width"
                )
                axs[chan].errorbar(
                    x + 0.5,
                    pos,
                ...
                axs[chan].set_title(
                    f'Pixel {pixid} {chan_str} Average Traces{suffix if suffix is not None else ""}'
                )
                axs[chan].grid()
    ...
    def ShowPedestal(self):
        try:
            ...
            if len(ped_shape) == 3:
                image = self.pos[0].mean(
                    axis=1
                )  # now that I've taken the HG, there is 2 axis : 0 for the pixel id, 1 for the slices
            else:
                image = self.pos[0]

            cam_disp = CalibrationCameraDisplay(
                geometry=GetCamera(),
                cmap="turbo",
                image=image,
                ax=axs[0][0],
                allow_pick=True,
                title="High-Gain Average Pedestal Position",
            )
            cam_disp.add_colorbar()
            cam_disp.set_function(self.ShowTraceInfo)

            if len(ped_shape) == 3:
                image = self.width[0].mean(
                    axis=1
                )  # now that I've taken the HG, there is 2 axis : 0 for the pixel id, 1 for the slices
            else:
                image = self.width[0]
            ...
            cam_disp = CalibrationCameraDisplay(
                ...
                ax=axs[0][1],
                allow_pick=True,
                title="High-Gain Average Pedestal Width",
            )
            cam_disp.add_colorbar()
            cam_disp.set_function(self.ShowTraceInfo)

            if len(ped_shape) == 3:
                image = self.pos[1].mean(
                    axis=1
                )  # now that I've taken the HG, there is 2 axis : 0 for the pixel id, 1 for the slices
            else:
                image = self.pos[1]
            ...
            cam_disp = CalibrationCameraDisplay(
                ...
                ax=axs[1][0],
                allow_pick=True,
                title="Low-Gain Average Pedestal Position",
            )
            cam_disp.add_colorbar()
            cam_disp.set_function(self.ShowTraceInfo)

            if len(ped_shape) == 3:
                image = self.width[1].mean(
                    axis=1
                )  # now that I've taken the HG, there is 2 axis : 0 for the pixel id, 1 for the slices
            else:
                image = self.width[1]
            ...
            cam_disp = CalibrationCameraDisplay(
                ...
                ax=axs[1][1],
                allow_pick=True,
                title="Low-Gain Average Pedestal Width",
            )
            cam_disp.add_colorbar()
            cam_disp.set_function(self.ShowTraceInfo)
...
class PedestalInfoOld:
    def __init__(self, camera):
        ...
        self.all_pos_per_slice = np.zeros(
            (2, camera.n_pixels)
        )  # std dev on the total trace
        self.all_width_per_slice = np.zeros(
            (2, camera.n_pixels)
        )  # std dev on the total trace
    ...
    def ShowHGPedestalDistribution(self, pixid):
        ...
        ax.set_xlabel("ADC")
        ax.set_title(
            f"Pixel {pixid} High Gain Pedestal Distribution\n$\mu$: {np.mean(datas):.2f}, $\sigma$: {np.std(datas):.2f}"
        )
        ax.grid()
        plt.show()
    ...
    def ShowLGPedestalDistribution(self, pixid):
        ...
        ax.set_xlabel("ADC")
        ax.set_title(
            f"Pixel {pixid} Low Gain Pedestal Distribution\n$\mu$: {np.mean(datas):.2f}, $\sigma$: {np.std(datas):.2f}"
        )
        ax.grid()
        plt.show()
    ...
    def ShowHGPedestalT0Distribution(self, pixid):
        ...
        ax.set_xlabel("T0")
        ax.set_title(
            f"Pixel {pixid} High Gain T0 Pedestal Distribution\n$\mu$: {np.mean(datas):.2f}, $\sigma$: {np.std(datas):.2f}"
        )
        ax.grid()
        plt.show()
    ...
    def ShowLGPedestalT0Distribution(self, pixid):
        ...
        ax.set_xlabel("ADC")
        ax.set_title(
            f"Pixel {pixid} Low Gain T0 Pedestal Distribution\n$\mu$: {np.mean(datas):.2f}, $\sigma$: {np.std(datas):.2f}"
        )
        ax.grid()
        plt.show()
    ...
    def ShowHGTraceInfo(self, pixid):
        ...
        axs[0].plot(
            x,
            self.all_pos_per_slice[0, pixid, :],
            label="Over trace",
            color="tab:orange",
        )
        ...
        axs[1].plot(
            x,
            self.all_width_per_slice[0, pixid, :],
            label="Over trace",
            color="tab:orange",
        )
    ...
    def ShowLGTraceInfo(self, pixid):
        ...
        axs[0].plot(
            x,
            self.all_pos_per_slice[1, pixid, :],
            label="Over trace",
            color="tab:orange",
        )
        ...
        axs[1].plot(
            x,
            self.all_width_per_slice[1, pixid, :],
            label="Over trace",
            color="tab:orange",
        )
    ...
    def DisplayTrace(self):
        ...
        image1[image1 == 0.0] = np.nan
        cam_disp1 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image1,
            ax=ax1,
            allow_pick=True,
            title="High-Gain Pedestal Width",
        )
        ...
        cam_disp2 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image2,
            ax=ax2,
            allow_pick=True,
            title="Low-Gain Pedestal Width",
        )
        ...
        image3[image3 == 0.0] = np.nan

        image3 = image3 - expected_t0

        cam_disp3 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image3,
            ax=ax3,
            allow_pick=True,
            title="High-Gain Pedestal T0 Position - Expectation",
            cmap="coolwarm",
        )
        ...
        image4[image4 == 0.0] = np.nan

        image4 = image4 - expected_t0width

        cam_disp4 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image4,
            ax=ax4,
            allow_pick=True,
            title="High-Gain Pedestal T0 Standard Deviation - Expectation",
            cmap="coolwarm",
        )
        ...
        image5[image5 == 0.0] = np.nan

        image5 = image5 - expected_t0

        cam_disp5 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image5,
            ax=ax5,
            allow_pick=True,
            title="Low-Gain Pedestal T0 Position - Expectation",
            cmap="coolwarm",
        )
        ...
        image6[image6 == 0.0] = np.nan

        image6 = image6 - expected_t0width

        cam_disp6 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image6,
            ax=ax6,
            allow_pick=True,
            title="Low-Gain Pedestal T0 Standard Deviation - Expectation",
            cmap="coolwarm",
        )
        ...
        image7 = (
            (np.sum(np.power(self.std_wvf[0, :, :], 2.0), axis=1) - 1)
            / np.power(self.all_width[0, :], 2.0)
        ) * (self.all_width[0, :] > 0.0)

        cam_disp7 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image7,
            ax=ax7,
            allow_pick=True,
            title="High-Gain (Sum Slice Variance - Global Variance)/Global Variance",
        )
        ...
        image8 = (
            (np.sum(np.power(self.std_wvf[1, :, :], 2.0), axis=1) - 1)
            / np.power(self.all_width[1, :], 2.0)
        ) * (self.all_width[1, :] > 0.0)

        cam_disp8 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image8,
            ax=ax8,
            allow_pick=True,
            title="Low-Gain (Sum Slice Variance - Global Variance)/Global Variance",
        )
    ...
    def Display(self):
        ...
        cam_disp1 = CalibrationCameraDisplay(
            ...
            ax=ax1,
            allow_pick=True,
            title="High-Gain Pedestal Position",
        )
        cam_disp1.add_colorbar()
        ...
        cam_disp3 = CalibrationCameraDisplay(
            ...
            ax=ax3,
            allow_pick=True,
            title="Low-Gain Pedestal Position",
        )
        cam_disp3.add_colorbar()
        ...
        cam_disp5 = CalibrationCameraDisplay(
            ...
            ax=ax5,
            allow_pick=True,
            title="High-Gain Pedestal T0 Position",
        )
        cam_disp5.add_colorbar()
        ...
        cam_disp6 = CalibrationCameraDisplay(
            ...
            ax=ax6,
            allow_pick=True,
            title="High-Gain Pedestal T0 Standard Deviation",
        )
        cam_disp6.add_colorbar()
        ...
        image7[image7 == 0.0] = np.nan
        cam_disp7 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image7,
            ax=ax7,
            allow_pick=True,
            title="Low-Gain Pedestal T0 Position",
        )
        cam_disp7.add_colorbar()
        ...
        image8[image8 == 0.0] = np.nan
        cam_disp8 = CalibrationCameraDisplay(
            geometry=self.camera,
            image=image8,
            ax=ax8,
            allow_pick=True,
            title="Low-Gain Pedestal T0 Standard Deviation",
        )
        cam_disp8.add_colorbar()
