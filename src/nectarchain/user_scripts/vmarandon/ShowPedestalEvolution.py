try:
    ...
    from Utils import CustomFormatter, GetCamera, GetDefaultDataPath
...
def ComputePedestalEvolution(run, path):
    ...
    for evt in tqdm(data):
        ...
        if use_db:
            ...
            for pix in range(evt.pedestal.tel[0].width.shape[1]):
                ...
                moduleId = pix // 7
                try:
                    temp1 = evt.db.tel[0]["monitoring_drawer_temperatures"][
                        moduleId
                    ].tfeb1
                ...
                try:
                    temp2 = evt.db.tel[0]["monitoring_drawer_temperatures"][
                        moduleId
                    ].tfeb2
                except KeyError as error:
                    temp2 = 0
    ...
    if use_db:
        ...
        temperatures1 = ma.array(temperatures1, mask=temperatures1 == 0.0)
        temperatures2 = ma.array(temperatures2, mask=temperatures2 == 0.0)
    ...
    if use_db:
        ...
        evo_info.intercepts_t1_pedmean = ma.array(
            intercepts_t1_pedmean, mask=mask_t1_pedmean
        )
        ...
        evo_info.intercepts_t2_pedmean = ma.array(
            intercepts_t2_pedmean, mask=mask_t2_pedmean
        )
...
def ShowPedesatalEvolution(arglist):
    p = argparse.ArgumentParser(
        description="Show the Pedestal Evolution for a given run",
