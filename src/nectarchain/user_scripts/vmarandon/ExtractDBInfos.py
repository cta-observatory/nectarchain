try:
    import sys
    import argparse
    from Utils import CustomFormatter, GetDefaultDataPath, GetDefaultDBPath, save_simple_data
...
def ExtractDBInfos(arglist):
    ...
    p = argparse.ArgumentParser(description='Dump db infos',
                                epilog='examples:\n'
                                '\t python %(prog)s --run 123456  \n',
                                formatter_class=CustomFormatter)

    p.add_argument("--run", dest='run', type=int, help="Run number for which to dump db infos")
    ...
    p.add_argument("--db-path",dest='dbPath',type=str,default=GetDefaultDBPath(),help="Path to the directory with db files. The program will recursively search in all directory below")
    ...
    if (end_time - begin_time).total_seconds()<60:
        ...
        end_time = begin_time + timedelta(seconds=60.)
    dbinfos = DBInfos.init_from_time(begin_time=begin_time,end_time=end_time,dbpath=args.dbPath,verbose=False)
    try:
