...
def ShowHardwareProblem(arglist):
    ...
    p.add_argument(
        ...
        dest="listEvent",
        action="store_true",
        help="List pixels for which there is hardware problems",
    )
    ...
    p.add_argument(
        ...
        dest="debug",
        action="store_true",
        help="Debug mode (only 10000 events processed)",
    )
    ...
    cam_count_hg = CameraDisplay(
        geometry=GetCamera(),
        cmap="turbo",
        image=ma.array(
            camstats.mean[0] * camstats.count[0], mask=camstats.mean[0] == 0.0
        ),
        title=f"HG Hardware Failing Counts\nrun {args.run}",
        ax=axs_counts[0],
        show_frame=False,
        allow_pick=True,
    )
    ...
    cam_count_lg = CameraDisplay(
        geometry=GetCamera(),
        cmap="turbo",
        image=ma.array(
            camstats.mean[1] * camstats.count[1], mask=camstats.mean[1] == 0.0
        ),
        title=f"LG Hardware Failing Counts\nrun {args.run}",
        ax=axs_counts[1],
        show_frame=False,
        allow_pick=True,
    )
    ...
    cam_frac_hg = CameraDisplay(
        geometry=GetCamera(),
        cmap="turbo",
        image=ma.array(camstats.mean[0] * 100, mask=camstats.mean[0] == 0.0),
        title=f"HG Hardware Failing Fraction (%)\nrun {args.run}",
        ax=axs_frac[0],
        show_frame=False,
        allow_pick=True,
    )
    ...
    cam_frac_hg.set_limits_minmax(0.0, 100.0)
    cam_frac_hg.colorbar.set_label("%")

    cam_frac_lg = CameraDisplay(
        geometry=GetCamera(),
        cmap="turbo",
        image=ma.array(camstats.mean[1] * 100, mask=camstats.mean[1] == 0.0),
        title=f"LG Hardware Failing Fraction (%)\nrun {args.run}",
        ax=axs_frac[1],
        show_frame=False,
        allow_pick=True,
    )
    ...
    cam_frac_lg.set_limits_minmax(0.0, 100.0)
