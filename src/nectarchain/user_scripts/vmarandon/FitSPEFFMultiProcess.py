try:
    ...
    from Utils import GetCamera, SignalIntegration, IntegrationMethod, ConvertTitleToName, CustomFormatter, GetDefaultDataPath, save_simple_data
...
def DoSPEFFFit(arglist):
    ...
    p.add_argument("--batch",dest='batch',action='store_true',help="run in batch, no plots will be displayed")
    p.add_argument("--t0-file",dest='t0file',type=str,default=None,help="file that contain the T0 position to use for each pixel")
    ...
    try:
        for evt in tqdm(data,total=nEvents):
            ...
            if evt.trigger.event_type == EventType.FLATFIELD:
                ...
                charges, times = SignalIntegration( wvf, exclusion_mask=hdw_fail_slice, peakpositions=t0max,method=IntegrationMethod.USERPEAK,left_bound=5,right_bound=11,camera=camera)
    ...
    cam_illu = CameraDisplay(geometry=GetCamera(),image=ma.array(cam_illu_values,mask=~valid_fit),cmap='turbo',title=f'Mean Illumination (VIM Edition)\nRun: {run}',show_frame=False)
    ...
    cam_ns = CameraDisplay(geometry=GetCamera(),image=ma.array(cam_ns_values,mask=~valid_fit),cmap='turbo',title=f'Ns Parameter (VIM Edition)\nRun: {run}',show_frame=False)
    ...
    fig9 = plt.figure(figsize=(8,8))
    cam_pedw = CameraDisplay(geometry=GetCamera(),image=ma.array(cam_pedw_values,mask=~valid_fit),cmap='turbo',title=f'Pedestal Width (VIM Edition)\nRun: {run}',show_frame=False)
    ...
    with open(outdataname,'w') as outfile:
        outfile.write(f"pix Ped SigmaPed Ns Illu Gain SigmaGain ErrorPed ErrorSigmaPed ErrorNs ErrorIllu ErrorGain ErrorSigmaGain Valid FVal NFcn\n")
        for p, f in pixelFitResult.items():
            ...
            outfile.write(f"{p} {ped} {sigmaped} {ns} {illu} {gain} {sigmagain} {err_ped} {err_sigmaped} {err_ns} {err_illu} {err_gain} {err_sigmagain} {f.minuit_result.valid} {f.minuit_result.fval} {f.minuit_result.nfcn}\n")
