...
def ExtractInformationSingleRun(
    run,
    data_path,
    dest_path,
    data_block,
    ...
    with DataDumper(run=run, path=dest_path, data_block=data_block) as dd:
        ...
        events = GetNectarCamEvents(
            run=run, path=data_path, data_block=data_block, applycalib=applycalib
        )
        ...
        for evt in tqdm(events):
            ...
            if doIntegration:
                ...
                charges, times = NeighborPeakIntegration(
                    waveform=waveform.copy(), camera=camera, bad_pix=bad_pix
                )  # copy as it can touch waveform
                charges[bad_pix] = 0
                times[bad_pix] = 0
...
def ExtractInformation(arglist):
    ...
    p = argparse.ArgumentParser(
        description="Extract R0 information from a run and split it in multiple sub-files to speed up exploration",
        epilog="examples:\n"
    ...
    p.add_argument(
        ...
        type=int,
        nargs="+",
        help="Run numbers to be converted. Can be one or more.",
    )
    p.add_argument(
        "--data-path",
        dest="dataPath",
    ...
    p.add_argument(
        ...
        type=str,
        default="None",
        help="Path to the destination directory where the extracted data will be written",
    )
    p.add_argument(
        "--apply-calib",
        dest="applyCalib",
        type=str,
        default="True",
        help="Don't apply the calibration coefficient. Useful when the channel selection has been done",
    )
    p.add_argument(
        "--keep-r1",
        dest="keepR1",
        type=str,
        default="True",
        help="Save the R1 data if True",
    )
    p.add_argument(
        "--split",
        dest="split",
        action="store_true",
        help="Split the files per groups. 0-1 together in a file, 2-3 in another, etc... Need the ctapipe_io_nectarcam version compatible with ctapipe 0.18",
    )
    p.add_argument(
        "--nnint",
        dest="nnint",
        action="store_true",
        help="Do an integration of the data using Next Neighbor Peak Search. At the moment hard coded to be 10 ns -4 and +6 ns after the max. Will create charge and TO data set",
    )
    p.add_argument(
        "--only-trigger",
        dest="onlytrig",
        action="store_true",
        help="Extract only the trigger information to a file. Useful for big runs",
    )
    ...
    for run in args.runs:
        ...
        if False and args.split:
            ...
            results = pool.starmap(
                ExtractInformationSingleRun,
                zip(runs, paths, dest_paths, blocks, calib, keepR1, nnints, trigonly),
            )
