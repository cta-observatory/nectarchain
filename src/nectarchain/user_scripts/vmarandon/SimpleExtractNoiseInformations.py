try:
    ...
    from ctapipe.core import TelescopeComponent, Tool, Container, Field, Map
    from ctapipe.instrument import SubarrayDescription
    from ctapipe.containers import ArrayEventContainer, EventType
    from ctapipe.core.traits import Long, Bool, Integer, Path
    ...
    from Utils import CustomFormatter, save_simple_data, GetEventTypeFromString
...
def SimpleExtractNoiseInformations(arglist):
    ...
    p.add_argument("--event-type",dest='eventType',type=str,default="SKY_PEDESTAL",help='Event type to be used for the pedestal extraction')
    p.add_argument("--max-event",dest="maxevent",type=int,default=-1,help="Max number of event to analyze")
    ...
    try:
        for ie, evt in enumerate(tqdm(events,total=nMaxEvent)):
            ...
            if evt.trigger.event_type == event_type:
                ...
                noiseinfos.std_noise_stats.add( std_wvf, validmask=(hdw_good & noisy_pixels) )

                if default_mask is None:
                    default_mask = np.full_like( wvf, fill_value=True,dtype=bool )
                
                noiseinfos.waveform.add( wvf, validmask=(default_mask & hdw_good[:,:,None]) )
                ...
                if default_fft_mask is None:
                    default_fft_mask = np.full_like( fft_norm2, fill_value=True,dtype=bool )
                ...
                good_hdw_fft_mask = good_hdw_fft_mask & ((np.min(fft_norm2,axis=-1)>1.e-3)[:,:,None]) ## Sometimes there is values close to 0 in the FFT, remove those
                noiseinfos.fft_all.add( fft_norm2, validmask=good_hdw_fft_mask)
                noiseinfos.fft_noise.add( fft_norm2, validmask=(good_hdw_fft_mask & noisy_pixels[:,:,None] ) )
                noiseinfos.fft_clean.add( fft_norm2, validmask=(good_hdw_fft_mask & clean_pixels[:,:,None] ) )
