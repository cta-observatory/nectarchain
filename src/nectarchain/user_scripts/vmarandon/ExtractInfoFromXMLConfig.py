...
def ExtractInfoFromXMLConfig(arglist):
    p = argparse.ArgumentParser(
        description="Extract target HV and current trip limit from config file",
        epilog="examples:\n"
    ...
    if len(pixels) != len(set(pixels)):
        print(
            "WARNING> It looks like there is multiple time the same pixel in the XML config.... YOU SHOULD CHECK !"
        )
