    ...
    def __get_selection_condition__(self, table):
        ...
        if self.time_start is not None:
            if isinstance(self.time_start, datetime.datetime):
                ...
                print(
                    f"WARNING> {self.time_start} of type {type(self.time_start)} is of a non handled type ==> Won't be used (please correct code)"
                )
        ...
        if self.time_end is not None:
            if isinstance(self.time_end, datetime.datetime):
                ...
                print(
                    f"WARNING> {self.time_end} of type {type(self.time_end)} is of a non handled type ==> Won't be used (please correct code)"
                )
    ...
    def set_db(self, dbname, path):
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Error connecting to sqlite for file [{dbname}] in path [{path}]"
            )
        ...
        for table in db_to_load:
            ...
            if flags & DBInfosFlag.CAMERA:
                if flags & DBInfosFlag.DRAWER and flags & DBInfosFlag.PIXEL:
                        ...
                        for pix in range(min_pix, max_pix + 1):
                            ...
                            if len(df_sel) > 0:
                                ...
                                if table not in self.telinfos[cam]:
                                    # print(f"THERE: {table}")
                                    self.telinfos[cam][table] = DBCameraElementInfos(
                                        times
                                    )
                                # print("HERE")
                    ...
                    end_time = time.time()
                    print(
                        f"Time to fill pixel structures for table [{table}]: {end_time-start_time}"
                    )
                    ...
                    end_time = time.time()
                    print(
                        f"Time to fill drawer structure for table [{table}]: {end_time-start_time}"
                    )
                else:
                    start_time = time.time()
                    ...
                    end_time = time.time()
                    print(
                        f"Time to fill camera structure for table [{table}]: {end_time-start_time}"
                    )
            else:
                start_time = time.time()
                ...
                end_time = time.time()
                print(
                    f"Time to fill structure for table [{table}]: {end_time-start_time}"
                )
    ...
    def __advance_to__(self, time):
        ...
        if self.times is not None:
            ...
            try:
                ...
                while (
                    self.times[self.index] < time and self.times[self.index + 1] < time
                ):
                    self.index += 1
...
class DBInfos:
    ...
    def __advance_to__(self, time):
        ...
        try:
            ...
            while self.current_time < time and (
                self.next_time is not None and self.next_time < time
            ):
                self.index += 1
                self.__loadtime__()
        except KeyError as err:
