...
class ChargeContainer:
    ...
    def from_waveforms(
        ...
        **kwargs,
        ...
        charge_hg, peak_hg = ChargeContainer.compute_charge(
            waveformContainer, constants.HIGH_GAIN, method, **kwargs
        )
        charge_hg = np.array(charge_hg, dtype=np.uint16)
        log.info(f"computing lg charge with {method} method")
        charge_lg, peak_lg = ChargeContainer.compute_charge(
            waveformContainer, constants.LOW_GAIN, method, **kwargs
        )
        ...
        chargeContainer = cls(
            ...
            peak_hg,
            peak_lg,
            waveformContainer.run_number,
            waveformContainer.pixels_id,
            waveformContainer.nevents,
            waveformContainer.npixels,
            method,
        )
    ...
    def write(self, path: Path, start=None, end=None, block=None, **kwargs):
        ...
        try:
            ...
            log.info(
                f"charge saved in {Path(path)}/charge_run{self.run_number}{suffix}.fits"
            )
        except OSError as e:
            log.warning(e)
    ...
    def from_file(path: Path, run_number: int, **kwargs):
        """load ChargeContainer from FITS file previously written with
        ChargeContainer.write() method
    ...
    def histo_hg(self, n_bins: int = 1000, autoscale: bool = True) -> np.ndarray:
        ...
        if autoscale:
            all_range = np.arange(
                np.uint16(np.min(self.charge_hg.T[~mask_broken_pix].T)) - 0.5,
            ...
            charge_ma = ma.masked_array(
                (
                    ...
                    @ np.ones((1, self.charge_hg.shape[1]))
                ).T,
                mask=np.zeros(
                    (self.charge_hg.shape[1], all_range.shape[0]), dtype=bool
                ),
            )
            ...
            hist = np.array(
                [
                    np.histogram(self.charge_hg.T[i], bins=n_bins)[0]
                    for i in range(self.charge_hg.shape[1])
                ]
            )
            charge = np.array(
                [
                    np.histogram(self.charge_hg.T[i], bins=n_bins)[1]
                    for i in range(self.charge_hg.shape[1])
                ]
            )
            charge_edges = np.array(
    ...
    def histo_lg(self, n_bins: int = 1000, autoscale: bool = True) -> np.ndarray:
        ...
        if autoscale:
            all_range = np.arange(
                np.uint16(np.min(self.charge_lg.T[~mask_broken_pix].T)) - 0.5,
                np.uint16(np.max(self.charge_lg.T[~mask_broken_pix].T)) + 1.5,
                1,
            )
            charge_ma = ma.masked_array(
                (
                    all_range.reshape(all_range.shape[0], 1)
                    @ np.ones((1, self.charge_lg.shape[1]))
                ).T,
                mask=np.zeros(
                    (self.charge_lg.shape[1], all_range.shape[0]), dtype=bool
                ),
            )
            ...
            start = time.time()
            _mask, hist_ma_data = make_histo(
                self.charge_lg.T, all_range, mask_broken_pix
            )  # , charge_ma.data, charge_ma.mask, hist_ma.data, hist_ma.mask)
            charge_ma.mask = np.logical_or(_mask, broxen_pixels_mask)
            hist_ma = ma.masked_array(hist_ma_data, mask=charge_ma.mask)
            ...
            hist = np.array(
                [
                    np.histogram(self.charge_lg.T[i], bins=n_bins)[0]
                    for i in range(self.charge_lg.shape[1])
                ]
            )
            charge = np.array(
                [
                    np.histogram(self.charge_lg.T[i], bins=n_bins)[1]
                    for i in range(self.charge_lg.shape[1])
                ]
            )
            charge_edges = np.array(
    ...
    def select_charge_hg(self, pixel_id: np.ndarray):
        ...
        return np.array(
            [
                self.charge_hg.T[np.where(self.pixels_id == pixel)[0][0]]
                for pixel in pixel_id[mask_contain_pixels_id]
            ]
        ).T
    ...
    def select_charge_lg(self, pixel_id: np.ndarray):
        ...
        return np.array(
            [
                self.charge_lg.T[np.where(self.pixels_id == pixel)[0][0]]
                for pixel in pixel_id[mask_contain_pixels_id]
            ]
        ).T
...
class ChargeContainers:
    ...
    def from_waveforms(cls, waveformContainers: WaveformsContainers, **kwargs):
        ...
        for i in range(waveformContainers.nWaveformsContainer):
            chargeContainers.append(
                ChargeContainer.from_waveforms(
                    waveformContainers.waveformsContainer[i], **kwargs
                )
            )
    ...
    def from_file(path: Path, run_number: int, **kwargs):
        ...
        if len(files) == 0:
            ...
            for file in files:
                cls.chargeContainers.append(
                    ChargeContainer.from_file(path, run_number, explicit_filename=file)
                )
            return cls
    ...
    def nevents(self):
        ...
        """
        return np.sum(
            [self.chargeContainers[i].nevents for i in range(self.__nChargeContainer)]
        )
    ...
    def merge(self) -> ChargeContainer:
        ...
        cls.charge_hg = np.concatenate(
            [chargecontainer.charge_hg for chargecontainer in self.chargeContainers],
            axis=0,
        )
        cls.charge_lg = np.concatenate(
            [chargecontainer.charge_lg for chargecontainer in self.chargeContainers],
            axis=0,
        )
        cls.peak_hg = np.concatenate(
            [chargecontainer.peak_hg for chargecontainer in self.chargeContainers],
            axis=0,
        )
        cls.peak_lg = np.concatenate(
            [chargecontainer.peak_lg for chargecontainer in self.chargeContainers],
            axis=0,
        )

        if np.all(
            [
                chargecontainer.run_number == self.chargeContainers[0].run_number
                for chargecontainer in self.chargeContainers
            ]
        ):
            cls._run_number = self.chargeContainers[0].run_number
        if np.all(
            [
                chargecontainer.pixels_id == self.chargeContainers[0].pixels_id
                for chargecontainer in self.chargeContainers
            ]
        ):
        ...
        if np.all(
            [
                chargecontainer.method == self.chargeContainers[0].method
                for chargecontainer in self.chargeContainers
            ]
        ):
            cls._method = self.chargeContainers[0].method
        cls._nevents = np.sum(
            [chargecontainer.nevents for chargecontainer in self.chargeContainers]
        )
        if np.all(
            [
                chargecontainer.npixels == self.chargeContainers[0].npixels
                for chargecontainer in self.chargeContainers
            ]
        ):
            cls._npixels = self.chargeContainers[0].npixels

        cls.ucts_timestamp = np.concatenate(
            [
                chargecontainer.ucts_timestamp
                for chargecontainer in self.chargeContainers
            ]
        )
        cls.ucts_busy_counter = np.concatenate(
            [
                chargecontainer.ucts_busy_counter
                for chargecontainer in self.chargeContainers
            ]
        )
        cls.ucts_event_counter = np.concatenate(
            [
                chargecontainer.ucts_event_counter
                for chargecontainer in self.chargeContainers
            ]
        )
        cls.event_type = np.concatenate(
            [chargecontainer.event_type for chargecontainer in self.chargeContainers]
        )
        cls.event_id = np.concatenate(
            [chargecontainer.event_id for chargecontainer in self.chargeContainers]
        )
        cls.trig_pattern_all = np.concatenate(
            [
                chargecontainer.trig_pattern_all
                for chargecontainer in self.chargeContainers
            ],
            axis=0,
        )
