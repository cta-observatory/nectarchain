...
class TestChargesContainer:
    ...
    def test_write_charge_container(self, tmp_path="/tmp"):
        ...
        assert (
            len(
                glob.glob(
                    f"{tmp_path}/charge_run{TestChargesContainer.run_number}.fits"
                )
            )
            == 1
        )
    ...
    def test_load_charge_container(self, tmp_path="/tmp"):
        ...
        loaded_charge_container = ChargesContainerIO.load(
            tmp_path, TestChargesContainer.run_number
        )
    ...
    def test_sort_charge_container(self):
        ...
        assert sorted_charge_container.event_id.tolist() == sorted(
