        ...
        with HDF5TableReader(path) as reader:
            if len(reader._h5file.root.__members__) > 1 and slice_index is None:
                log.info(
                    ...
                    f"{len(reader._h5file.root.__members__)} slices, will return a "
                    f"generator"
                )
                        ...
                        try:
                            ...
                            if len(_waveforms_data) == 1:
                                ...
                                log.info(
                                    ...
                                    f"corresponding to a {container_class} table save, "
                                    f"unable to load"
                                )
                ...
                if slice_index is None:
                    log.info(
                        f"reading {container_class.__name__} containing a single slice,"
