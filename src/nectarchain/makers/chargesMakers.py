...
class ChargesMaker(ArrayDataMaker) : 
    ...
    """
#constructors
    def __init__(self,run_number : int,max_events : int = None,run_file:str = None,*args,**kwargs):
        """construtor
    ...
    def make(self, n_events=np.inf, trigger_type: list = None, restart_from_beginning: bool = False, method: str = "FullWaveformSum", *args, **kwargs):
        """
        ...
        return super().make(n_events=n_events, trigger_type=trigger_type, restart_from_beginning=restart_from_beginning, *args, **kwargs)
    ...
    def _make_output_container(self,trigger_type  : EventType, method : str,*args,**kwargs) :
        """
    ...
    def charges_hg(self,trigger : EventType) : 
        ...
        """
        return np.array(self.__charges_hg[__class__._get_name_trigger(trigger)],dtype = np.uint16)
    def charges_lg(self,trigger : EventType) : 
        ...
        Returns:
            np.ndarray: The charges for the specific trigger type.
        """
        return np.array(self.__charges_lg[__class__._get_name_trigger(trigger)],dtype = np.uint16)
    def peak_hg(self,trigger : EventType) : 
        ...
        """
        return np.array(self.__peak_hg[__class__._get_name_trigger(trigger)],dtype = np.uint16)
    def peak_lg(self,trigger : EventType) : 
        ...
        Returns:
            np.ndarray: The peak charges for the specific trigger type.
        """
        return np.array(self.__peak_lg[__class__._get_name_trigger(trigger)],dtype = np.uint16)
    ...
    def histo_hg(chargesContainer: ChargesContainer, n_bins: int = 1000, autoscale: bool = True) -> ma.masked_array:
        ...
        """
        return __class__._histo(chargesContainer=chargesContainer, field='charges_hg', n_bins=n_bins, autoscale=autoscale) 
    ...
    def histo_lg(chargesContainer: ChargesContainer, n_bins: int = 1000, autoscale: bool = True) -> ma.masked_array:
        ...
        Returns:
            ma.masked_array: A masked array representing the charge histogram, where each row corresponds to an event and each column corresponds to a bin in the histogram.
        """
        return __class__._histo(chargesContainer=chargesContainer, field='charges_lg', n_bins=n_bins, autoscale=autoscale) 
    ...
    def _histo(chargesContainer: ChargesContainer, field: str, n_bins: int = 1000, autoscale: bool = True) -> ma.masked_array:
        ...
        if autoscale: 
            ...
            start = time.time()
            _mask, hist_ma_data = make_histo(chargesContainer[field].T, all_range, mask_broken_pix)
