...
import pytest
import traitlets
from ctapipe.containers import EventType, TriggerContainer
...
class TestEventsLoopNectarCAMCalibrationTool(TestBaseNectarCAMCalibrationTool):
    ...
    def test_init_with_output_path(self):
        ...
        tool_instance = EventsLoopNectarCAMCalibrationTool(
            run_number=self.RUN_NUMBER, output_path=custom_path
        )
        assert tool_instance.output_path == custom_path

    def test_init_with_max_events(self):
        tool_instance = EventsLoopNectarCAMCalibrationTool(
            run_number=self.RUN_NUMBER, max_events=10
        )
        assert tool_instance.max_events == self.MAX_EVENTS

    def test_init_with_events_per_slice(self):
        tool_instance = EventsLoopNectarCAMCalibrationTool(
            run_number=self.RUN_NUMBER, events_per_slice=self.EVENTS_PER_SLICE
        )
    ...
    def test_load_eventsource(self, tool_instance_run_file):
        ...
        assert isinstance(
            tool_instance_run_file.event_source, LightNectarCAMEventSource
        )
    ...
    def test_init_writer_full_mode(
        self, mock_makedirs, mock_remove, mock_writer, tool_instance
    ):
        ...
        mock_makedirs.assert_called_once_with(
            tool_instance.output_path.parent, exist_ok=True
        )
    ...
    @patch("nectarchain.makers.core.os.remove")
    @patch("nectarchain.makers.core.os.makedirs")
    def test_init_writer_sliced_mode(
        self, mock_makedirs, mock_remove, mock_writer, tool_instance
    ):
        tool_instance.overwrite = True
    ...
    def test_setup_eventsource(self, tool_instance_run_file):
        ...
        assert (
            tool_instance_run_file._npixels
            == tool_instance_run_file.event_source.nectarcam_service.num_pixels
        )
        assert np.all(
            tool_instance_run_file._pixels_id
            == tool_instance_run_file.event_source.nectarcam_service.pixel_ids
        )
        assert isinstance(
            tool_instance_run_file.event_source, LightNectarCAMEventSource
        )
    ...
    @patch(
        "nectarchain.makers.core.ComponentUtils.get_class_name_from_ComponentName",
        return_value="ValidComponentClass",
    )
    @patch(
        "nectarchain.makers.core.ComponentUtils.get_configurable_traits",
        return_value={"trait1": "value1"},
    )
    def test_get_provided_component_kwargs(
        self, mock_get_class_name, mock_get_valid_component, tool_instance
    ):
        tool_instance.trait1 = "value1"
        output_component_kwargs = tool_instance._get_provided_component_kwargs(
            "componentName"
        )
    ...
    @patch(
        "nectarchain.makers.core.get_valid_component",
        return_value=["WaveformsComponent"],
    )
    @patch(
        "nectarchain.makers.core.ComponentUtils.get_class_name_from_ComponentName",
        return_value="WaveformsComponentClass",
    )
    @patch(
        "nectarchain.makers.core.ComponentUtils.get_configurable_traits",
        return_value={"trait1": "value1"},
    ...
    def test_setup_components(
        self,
        mock_get_configurable_traits,
        mock_get_class_name,
        mock_get_valid_component,
        mock_component,
        tool_instance_run_file,
    ):
        ...
        assert (
            tool_instance_run_file.components[0]
            == mock_component.from_name.return_value
        )

    @patch("nectarchain.makers.core.os.remove")
    @patch(
        "nectarchain.makers.core.EventsLoopNectarCAMCalibrationTool._setup_eventsource"
    )
    @patch(
        "nectarchain.makers.core.EventsLoopNectarCAMCalibrationTool._setup_components"
    )
    @patch("nectarchain.makers.core.EventsLoopNectarCAMCalibrationTool._init_writer")
    def test_setup(
        self,
        mock_init_writer,
        mock_setup_components,
        mock_setup_eventsource,
        mock_remove,
        tool_instance_run_file,
    ):
    ...
    def test_start_sliced(
        self,
        mock_setup_components,
        mock_finish_components,
        mock_component,
        tool_instance_run_file,
    ):
        tool_instance_run_file.overwrite = True
    ...
    def test_finish(
        self, mock_finish_components, mock_component, tool_instance_run_file
    ):
        tool_instance_run_file.overwrite = True
        tool_instance_run_file.setup()
        tool_instance_run_file.components = [mock_component.from_name.return_value]
    ...
    def test_write_container_with_nectarcam_container(
        self, mock_writer, tool_instance_run_file
    ):
    ...
    @patch("nectarchain.makers.core.HDF5TableWriter")
    def test_write_container_with_triggermap_container(
        self, mock_writer, tool_instance_run_file
    ):
        tool_instance_run_file.writer = mock_writer
    ...
    @patch("nectarchain.makers.core.HDF5TableWriter")
    def test_write_container_with_invalid_container(
        self, mock_writer, tool_instance_run_file
    ):
        tool_instance_run_file.writer = mock_writer
    ...
    @patch("nectarchain.makers.core.HDF5TableWriter")
    def test_write_container_with_generic_exception(
        self, mock_writer, tool_instance_run_file
    ):
        tool_instance_run_file.writer = mock_writer
        container = MagicMock(spec=NectarCAMContainer)
...
class TestDelimiterLoopNectarCAMCalibrationTool:
    def test_split_run(self):
        ...
        event = NectarCAMDataContainer(
            trigger=TriggerContainer(event_type=EventType.FLATFIELD)
        )
        assert not (tool.split_run(event=event))
        event = NectarCAMDataContainer(
            trigger=TriggerContainer(event_type=EventType.UNKNOWN)
        )
