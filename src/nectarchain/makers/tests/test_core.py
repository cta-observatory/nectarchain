...
class TestArrayDataMaker:
    ...
    def test_merge(self) : 
        chargesMaker = ChargesMaker(run_number = TestArrayDataMaker.run_number,
                                max_events = TestArrayDataMaker.max_events)
        charges_1 = chargesMaker.make()
        chargesMaker_2 = ChargesMaker(run_number = TestArrayDataMaker.run_number,
                        max_events = TestArrayDataMaker.max_events)
    ...
    def test_merge_different_container(self) : 
        chargesMaker = ChargesMaker(run_number = TestArrayDataMaker.run_number,
                                max_events = TestArrayDataMaker.max_events)
        charges_1 = chargesMaker.make()
        wfsMaker_2 = WaveformsMaker(run_number = TestArrayDataMaker.run_number,
                        max_events = TestArrayDataMaker.max_events)
