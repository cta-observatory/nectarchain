...
class TestWaveformsNectarCAMCalibrationTool : 
    ...
    def general_structure_testing(self,output : WaveformsContainer,nevents:int,n_pixels:int,run_number:int) :
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    if _events_per_slice is not None and nevents%_events_per_slice == 0 :
                        ...
                        if _events_per_slice is None and _max_events is None:
                            ...
                            assert int(10*output.wfs_lg.mean()) == self.RUNS["wfs_lg_mean"][i]
                            assert int(10*output.wfs_lg.std()) ==self.RUNS["wfs_lg_std"][i]
                            assert output.wfs_hg.min() == self.RUNS["wfs_hg_min"][i]
                            assert output.wfs_hg.max() == self.RUNS["wfs_hg_max"][i]
                            assert int(10*output.wfs_hg.mean()) == self.RUNS["wfs_hg_mean"][i]
                            assert int(10*output.wfs_hg.std()) ==self.RUNS["wfs_hg_std"][i]
