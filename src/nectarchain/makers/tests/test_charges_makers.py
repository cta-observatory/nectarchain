...
class TestChargesNectarCAMCalibrationTool:
    ...
    def general_structure_testing(
        self, output: ChargesContainer, nevents: int, n_pixels: int, run_number: int
    ):
        ...
        assert output.charges_hg.dtype == np.float32
        assert output.charges_lg.dtype == np.float32
        ...
        assert output.peak_hg.dtype == np.float32
        assert output.peak_lg.dtype == np.float32
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                if _events_per_slice is not None and nevents % _events_per_slice == 0:
                    ...
                    if _events_per_slice is None and _max_events is None:
                        ...
                        assert (
                            int(10 * output.charges_lg.mean())
                            == self.RUNS["charges_lg_mean"][i]
                        )
                        assert (
                            int(10 * output.charges_lg.std())
                            == self.RUNS["charges_lg_std"][i]
                        )
                        assert output.charges_hg.min() == self.RUNS["charges_hg_min"][i]
                        assert output.charges_hg.max() == self.RUNS["charges_hg_max"][i]
                        assert (
                            int(10 * output.charges_hg.mean())
                            == self.RUNS["charges_hg_mean"][i]
                        )
                        assert (
                            int(10 * output.charges_hg.std())
                            == self.RUNS["charges_hg_std"][i]
                        )
                # Check output on disk
                assert Path(outfile).exists()
                chargesContainers = ChargesContainers.from_hdf5(outfile)
                for ncontainers, container in enumerate(chargesContainers):
