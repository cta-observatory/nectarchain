...
from nectarchain.data.container import WaveformsContainer,WaveformsContainerIO,ArrayDataContainer
...
class TestWaveformsMaker:
    ...
    def test_shape_valid(self):
        ...
        assert waveformsContainer.ucts_timestamp.shape == (waveformsContainer.nevents,) 
        assert waveformsContainer.ucts_busy_counter.shape == (waveformsContainer.nevents,)
        assert waveformsContainer.ucts_event_counter.shape == (waveformsContainer.nevents,) 
        ...
        assert waveformsContainer.trig_pattern_all.shape[0] == waveformsContainer.nevents
        ...
        assert waveformsContainer.wfs_hg.shape == (waveformsContainer.nevents,waveformsContainer.npixels,waveformsContainer.nsamples)
        assert waveformsContainer.wfs_lg.shape == (waveformsContainer.nevents,waveformsContainer.npixels,waveformsContainer.nsamples)
        assert waveformsContainer.broken_pixels_hg.shape == (waveformsContainer.nevents,waveformsContainer.npixels)
    ...
    def test_select_waveforms_hg(self) : 
        ...
        assert isinstance(WaveformsMaker.select_waveforms_hg(waveformsContainer_list[0],pixel_id),np.ndarray)
        assert isinstance(WaveformsMaker.select_waveforms_lg(waveformsContainer_list[0],pixel_id),np.ndarray)
    ...
    def test_create_from_events_list(self) : 
        ...
        waveformsContainer = WaveformsMaker.create_from_events_list(events_list,
