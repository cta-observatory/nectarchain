...
#methods
    ...
    @staticmethod
    def _update_parameters(parameters: Parameters, charge: np.ndarray, counts: np.ndarray, **kwargs) -> Parameters:
        """
        ...
        try:
            ...
            if (coeff_mean[1] - pedestal.value < 0) or ((coeff_mean[1] - coeff_mean[2]) - pedestal.max < 0):
    ...
    @staticmethod
    def _get_mean_gaussian_fit(charge: np.ndarray, counts: np.ndarray, extension: str = "", **kwargs) -> Tuple[np.ndarray, np.ndarray]:
        """
...
#constructors
    def __init__(self, charge, counts, *args, **kwargs) -> None:
        ...
        self._results.add_column(Column(np.zeros((self.npixels), dtype=np.float64), "likelihood",
                                        unit=u.dimensionless_unscaled))
        self._results.add_column(Column(np.zeros((self.npixels), dtype=np.float64), "pvalue",
                                        unit=u.dimensionless_unscaled))
    @classmethod
...
#methods
    ...
    @staticmethod
    def _NG_Likelihood_Chi2(pp : float,res: float,mu2: float,n: float,muped: float,sigped: float,lum: float,charge : np.ndarray,counts:np.ndarray,**kwargs) : 
        """
    ...
    def cost(charge:np.ndarray,counts:np.ndarray) : 
        ...
        """
        def Chi2(pedestal: float,pp: float,luminosity: float,resolution: float,mean: float,n: float,pedestalWidth: float) :
    ...
    def _make_fit_array_from_parameters(self, pixels_id : np.ndarray = None, **kwargs) -> np.ndarray:
        """
    ...
    def plot_single(pixel_id: int, charge: np.ndarray, counts: np.ndarray, pp: float, resolution: float, gain: float, gain_error: float, n: float, pedestal: float, pedestalWidth: float, luminosity: float, likelihood: float) -> tuple:
        """
...
class FlatFieldSingleNominalSPEMaker(FlatFieldSingleHHVSPEMaker):
    ...
    def __init__(self, charge:np.ndarray, counts:np.ndarray, nectarGainSPEresult: str, same_luminosity: bool = False, *args, **kwargs) -> None:
        """
    ...
    @staticmethod
    def _update_parameters(parameters:Parameters, charge:np.ndarray, counts:np.ndarray, pixel_id, nectarGainSPEresult:QTable, **kwargs):
        """
