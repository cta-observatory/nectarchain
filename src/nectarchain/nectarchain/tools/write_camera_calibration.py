...
"""
import numpy as np
from traitlets import Dict, List, Unicode, Float
...
class CalibrationHDF5Writer(Tool):
    ...
    def start(self):
        ...
        for count, event in enumerate(self.eventsource):
            ...
            elif (event.r1.tel[self.tel_id].trigger_type == 5 or event.r1.tel[self.tel_id].trigger_type == 4) and (
                    ped_initialized and
                    np.median(np.sum(event.r1.tel[self.tel_id].waveform[0, ids], axis=1)) > self.minimum_charge):
