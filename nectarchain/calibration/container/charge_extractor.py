...
class gradient_extractor(ImageExtractor) : 
    ...
    def __call__(self, waveforms, telid, selected_gain_channel,substract_ped = False):
        ...
        if substract_ped : 
            ...
            #calculate pedestal
            ped_mean = np.mean(waveforms[:16])

            y = waveforms - ped_mean # waveforms without pedestal
        else : 
        ...
        yi = ius(xi)
        peaks, _ = find_peaks(yi, height=self.height_peak)
        ...
        if len(peaks)>0:
            ...
            if (xi[peaks[max_peak_index]]>20)&(xi[peaks[max_peak_index]]<40):                                                                     # Search the adaptive integration window
                ...
                window_right = (self.fixed_window-6)/2
                window_left = (self.fixed_window-6)/2 + 6
                charge_integral_fixed_window = ius.integral(xi[peaks[max_peak_index]-(window_left)], xi[peaks[max_peak_index]+window_right])
                charge_sum_fixed_window = yi[(peaks[max_peak_index]-(window_left)):peaks[max_peak_index]+window_right].sum()/(self.fixed_window)    # simple sum integration
